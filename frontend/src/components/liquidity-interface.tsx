...
import { usePoolData } from "@/hooks/use-pool";
...
const LiquidityInterface = () => {
  ...
  const campBalance = useTokenBalance(TOKEN.CAMP);
  const usdcBalance = useTokenBalance(TOKEN.USDC);
  const userPosition = getUserPosition();

  const [liquidityData, setLiquidityData] =
    useState<
      ReturnType<ReturnType<typeof useLiquidity>["calculateAddLiquidity"]>
    >(null);
  ...
  useEffect(() => {
    if (
      ...
      const data = calculateAddLiquidity(amountA, amountB, "CAMP", "USDC");
  ...
  const handleMaxA = () => {
    const balance =
      Number(campBalance.balance) / Math.pow(10, TOKEN.CAMP.decimals);
    handleAmountAChange(balance.toString());
  };

  const handleMaxB = () => {
    const balance =
      Number(usdcBalance.balance) / Math.pow(10, TOKEN.USDC.decimals);
  ...
  const isInsufficientBalance = () => {
    ...
    const campAmount = parseFloat(amountA) * Math.pow(10, TOKEN.CAMP.decimals);
    const usdcAmount = parseFloat(amountB) * Math.pow(10, TOKEN.USDC.decimals);
