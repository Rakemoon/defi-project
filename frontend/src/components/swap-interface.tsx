...
const SwapInterface = () => {
  ...
  const [tokenOut, setTokenOut] = useState<keyof typeof TOKEN>("USDC");
  const [amountIn, setAmountIn] = useState("");
  const [slippageTolerance, setSlippageTolerance] = useState(0.5);
  const [showSettings, setShowSettings] = useState(false);

  const { calculateSwap, executeSwap, isSwapping } = useSwap();
  const tokenInBalance = useTokenBalance(TOKEN[tokenIn]);
  const tokenOutBalance = useTokenBalance(TOKEN[tokenOut]);
  ...
  const handleMaxClick = () => {
    const balance =
      Number(tokenInBalance.balance) / Math.pow(10, TOKEN[tokenIn].decimals);
  ...
  const isInsufficientBalance = () => {
    ...
    const inputAmount =
      parseFloat(amountIn) * Math.pow(10, TOKEN[tokenIn].decimals);
            ...
            >
              <div className="flex justify-between items-center mb-2">
                ...
                <span
                  ...
                  style={{ color: "rgba(251, 250, 249, 0.7)" }}
                >
                  Balance: {formatTokenAmount(tokenInBalance.balance, tokenIn)}
                </span>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
                <input
                  ...
                  placeholder="0.0"
                  className="flex-1 rounded-md bg-transparent text-lg sm:text-2xl font-bold outline-none input-primary min-w-0 p-2"
                  style={{ color: "#FBFAF9" }}
                />
          ...
          >
            ...
            <div className="flex justify-between text-xs sm:text-sm">
              ...
              <span className="truncate ml-2" style={{ color: "#FBFAF9" }}>
                {swapData.fee} {TOKEN[tokenIn].symbol}
              </span>
            </div>
            <div className="flex justify-between text-xs sm:text-sm">
              <span style={{ color: "rgba(251, 250, 249, 0.7)" }}>
